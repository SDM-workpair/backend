...
class CRUDMR_Member_Tag(
    CRUDBase[MR_Member_Tag, MR_Member_Tag_Create, MR_Member_Tag_Update]
):
    def get_1st_self_tag_by_member_id(
        ...
        return (
            ...
            .filter(
                MR_Member_Tag.member_id == member_id, MR_Member_Tag.is_self_tag == True
            )
            .first()
        )
    ...
    def get_1st_find_tag_by_member_id(
        ...
        return (
            db.query(MR_Member_Tag)
            .filter(
                MR_Member_Tag.member_id == member_id, MR_Member_Tag.is_find_tag == True
            )
            .first()
        )
    ...
    def create_tag(
        ...
        *,
        ...
        mr_member_tag_in: MR_Member_Tag_Create
    ) -> MR_Member_Tag:
        # Get matching_room uuid
        matching_room_in = crud.matching_room.get_by_room_id(
            db=db, room_id=mr_member_tag_in.matching_room.room_id
        )
